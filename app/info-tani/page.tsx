...
function CatalogSlotCard({ slot }: { slot: ReturnType<typeof sampleCatalogs>[number] }) {
  ...
  const displayName = profile?.farmerName?.trim() || slot.name;
  ...
  const description = profile?.description?.trim() || (products.length > 0
    ? "Data katalog ini mengikuti produk yang tersimpan di database."
    : "Admin belum mengisi deskripsi, produk, dan foto untuk slot katalog ini.");
    ...
    <main className="min-h-screen bg-cyan-100/55 pb-14 pt-4">
      <section className="mx-auto w-full max-w-6xl rounded-3xl border border-cyan-200 bg-cyan-100/70 p-5 shadow-sm sm:p-7 lg:p-8">
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {catalogs.map((slot) => {
            return <CatalogSlotCard key={slot.id} slot={slot} />;
          })}
        </div>
      </section>
    </main>
