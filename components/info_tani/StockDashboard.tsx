...
export default function StockDashboard() {
  ...
  const totalStock = STOCK_KOMODITAS.reduce(
    (sum, item) => sum + item.stok_kg,
    0,
  );
  const readyItems = STOCK_KOMODITAS.filter(
    ...
    <>
      ...
      <section className="rounded-3xl border border-cyan-200 bg-white p-5 shadow-sm sm:p-6">
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-4">
          <div className="rounded-2xl bg-emerald-50 p-3 sm:p-4">
            ...
            <p className="mt-1 text-xl font-bold text-emerald-900 sm:text-2xl">
              {readyItems}/{STOCK_KOMODITAS.length}
            </p>
          </div>
        ...
        <button
          ...
          className="mt-5 w-full rounded-xl bg-gradient-to-r from-cyan-600 to-cyan-700 px-4 py-3 font-semibold text-white transition hover:shadow-lg hover:brightness-110 flex items-center justify-center gap-2"
        >
          <TrendingUp className="h-5 w-5" />
          Lihat Detail Stok (5 Komoditas)
        </button>
      </section>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-cyan-200 bg-white shadow-2xl">
            {/* Modal Header */}
            <div className="sticky top-0 flex items-center justify-between border-b border-cyan-200 bg-gradient-to-r from-cyan-50 to-cyan-100 px-5 py-4 sm:px-6">
              <div>
                <h3 className="text-lg font-semibold text-slate-900">
                  Detail Stok Komoditas
                </h3>
                <p className="mt-0.5 text-sm text-slate-600">
                  {STOCK_KOMODITAS.length} jenis komoditas
                </p>
              </div>
              <button
            ...
            <div className="space-y-3 p-5 sm:p-6">
              {STOCK_KOMODITAS.map((item) => (
                <article
                  key={item.id}
                  className="overflow-hidden rounded-2xl border border-cyan-100 bg-gradient-to-r from-cyan-50/50 to-white p-4 transition hover:shadow-md"
                >
                  <div className="flex gap-4">
                    {/* Gambar */}
                    <div className="relative h-20 w-20 flex-shrink-0 overflow-hidden rounded-lg bg-slate-200 sm:h-24 sm:w-24">
                      <Image
